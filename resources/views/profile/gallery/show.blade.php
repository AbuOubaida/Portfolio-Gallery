...
@section('content')
    <div class="callout primary">
        <article class="grid-container">
            <div class="grid-x grid-margin-x">
                <div class="medium-7">
                    <p><a href="{{url('/profile')}}"><i class="fa fa-arrow-circle-left"></i> Back to Portfolio</a></p>
                    @if($owner)
                    ...
                    <a href="{{url('/')}}/profile/photo/create/{{$gallery->id}}" class="button"><i class="	fa fa-upload"></i> Upload Portfolio</a><br><br>
                    <a href="{{url('/')}}/gallery/edit/{{$gallery->id}}"><button class="button bg-success"><i class="fa fa-edit"></i> Edit</button></a>
                    <a href="{{url('/')}}/gallery/destroy/{{$gallery->id}}" onclick="return confirm('Are you sure to delete? If you delete this gallery then also delete this related all photos')"><button class="button bg-danger margin-0"><i class="fa fa-trash"></i> Delete</button></a>
                </div>
                <div class="medium-5">
                    <div class="cell">
    ...
    <article class="grid-container">
        ...
        <div class="grid-x grid-margin-x small-up-2 medium-up-3 large-up-4">
            @foreach($photo as $p)
                <div class="cell">
                    <a href="{{url('/')}}/profile/photo/details/{{$p->id}}">
                        <img class="thumbnail cover-image" src="{{url('/')}}/images/{{$p->image}}">
                    </a>
                    <h5>{{$p->title}}</h5>
                    <p class="text-justify">{{substr($p->description,0,75)}}......<a href="{{url('/')}}/profile/photo/details/{{$p->id}}">More</a></p>
                </div>
            @endforeach
        </div>
        @else
